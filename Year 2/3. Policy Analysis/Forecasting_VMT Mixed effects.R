...
write.csv(subset(norm_scale, select = c(Full_FIPS_Code, YEAR, County_Type, Log_predictions, Predictions,
                                        ...
                                        LOGPOP_DENSITY)), 'VMT Modeling Dataset and results.csv')
...
all_forecast_preds <- predict(best_model, as.matrix(subset(norm_scale, select = -c(County_Type, Full_FIPS_Code, YEAR, LOGVMT))))
...
write.csv(subset(norm_scale, select = c( YEAR, Full_FIPS_Code,fips_year, Log_predictions, Predictions)), 'VMT Forecast Dataset and results.csv')
