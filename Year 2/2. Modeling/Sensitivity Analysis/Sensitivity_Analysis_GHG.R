...
# FHWA Mobility Trends Year 2 Sensitivity Analysis - GHG
# Date: March 20, 2024
...
performance_metric <-  "TOTAL_EMISSIONS" #set your dependent variable 
...
lasso_x1 <- subset(train, select = -c(LOGTOTAL_EMISSIONS,Full_FIPS_Code, YEAR, County_Type))
...
best_model <- read_rds(paste(dirname(getwd()),"/Modeling/champion_GHG_y2_2_16_24.rds", sep = ''))
...
df_sensitivity_dropna <- drop_na(df_sensitivity[,c("POPULATION","True_GDP","Unemployment_Rate", "Charging_Stations","LNMILES",
                    "TELEWORK","UPT_distr_commuters", "COURIER_NONEMP_RCPTOT_REAL", "DRIVER_NONEMP_RCPTOT_REAL", "POP_DENSITY", "TOTAL_EMISSIONS", "Full_FIPS_Code", "YEAR")]) #removed year and gems geotype
df_sensitivity_dropna <- df_sensitivity_dropna[!(df$TOTAL_EMISSIONS == 0),]
...
sensitivty_processs <- function(data_point_sensitivity){
  ...
  data_point_sensitivity$LOGTOTAL_EMISSIONS <- log(data_point_sensitivity$TOTAL_EMISSIONS + 1)
  ...
  data_point_sensitivity <- subset(data_point_sensitivity, select = -c(TOTAL_EMISSIONS, POPULATION, True_GDP, LNMILES, Charging_Stations, UPT_distr_commuters, Unemployment_Rate, TELEWORK, COURIER_NONEMP_RCPTOT_REAL, DRIVER_NONEMP_RCPTOT_REAL, POP_DENSITY))
  ...
  data_point_sensitivity <- subset(data_point_sensitivity, select = -c(LOGTOTAL_EMISSIONS, YEAR, Full_FIPS_Code))
...
percent_100_data_point <- df_sensitivity_one_hot[which.min(abs(quantile_values[2] - df_sensitivity_one_hot$Unemployment_Rate)),]
percent_100_data_point_adjusted <- percent_100_data_point
...
#UPT
quantile_values <- quantile(df_sensitivity_one_hot$UPT_distr_commuters) 
percent_100_data_point <- df_sensitivity_one_hot[which.min(abs(quantile_values[5] - df_sensitivity_one_hot$UPT_distr_commuters)),]
percent_100_data_point_adjusted <- percent_100_data_point
...
write.csv(sensitivity_analysis, "sensitivity_analysis_mixed_effectsGHG.csv", row.names=FALSE)
