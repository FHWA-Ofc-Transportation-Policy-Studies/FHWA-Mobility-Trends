...
# FHWA Mobility Trends Year 2 Sensitivity Analysis - VMT
# Date: March 20, 2024
...
df_sensitivity_dropna <- df_sensitivity_dropna[!(df_sensitivity_dropna$VMT == 0),]
df_year_subset <- subset(df_sensitivity_dropna, select=c(YEAR))
df_sensitivity_dropna$Full_FIPS_Code <- as.factor(df_sensitivity_dropna$Full_FIPS_Code)
counties_onehot <- data.frame(predict(dum_variables, newdata = df_sensitivity_dropna))
